<template>
  ...
  >
    <defs>
      ...
      <radialGradient :id="`${props.gradientId}-top`" cx="50%" cy="0%" r="150%">
        <template v-for="(color, index) in topColors" :key="index">
          <stop
            :offset="`${(index / (topColors.length - 1)) * 80}%`"
            :stop-color="color"
          />
        </template>
      </radialGradient>
      ...
      >
        <template v-for="(color, index) in bottomColors" :key="index">
          <stop
            :offset="`${(index / (bottomColors.length - 1)) * 100}%`"
            :stop-color="color"
          />
        </template>
      </radialGradient>
    ...
    <!-- Top Half -->
    <path
      d="M 0,50 A 50,50 0 1,0 100,50 Z"
      :fill="`url(#${props.gradientId}-bottom)`"
    />

    <!-- Bottom Half -->
    <path
      d="M 100,50 A 50,50 0 1,0 0,50 Z"
      :fill="`url(#${props.gradientId}-top)`"
    />
  </svg>
</template>
...
const bottomColors = computed(() =>
  props.colors.slice(midpoint - 1, props.colors.length)
)
</script>
